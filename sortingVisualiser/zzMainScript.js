...
let root, openCIBtn, colorInfoWrap, selectSpeed, speedValue;
...
window.onload = () => {
    ...
    openCIBtn = document.getElementById('openColorInfo');
    colorInfoWrap = document.getElementById('colorInfoWrapper');
    ...
    /* change sorting speed based on the selection */
    selectSpeed = document.getElementById('speedSelector');
    speedValue = document.getElementById('speedValue');
    selectSpeed.addEventListener('input', (ev) => {
        let rangeMax = parseInt(ev.target.max) + 1;
        let [currValue, rangeHalf] = [parseInt(ev.target.value), rangeMax / 2];
        let speed = (2 ** (rangeHalf - currValue));
        delay = baseDelay * speed;
        speedValue.textContent = 1 / speed;
        //console.log(currValue, rangeMax, rangeHalf, delay);
    });
